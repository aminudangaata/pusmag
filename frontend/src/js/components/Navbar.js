...
export function Navbar(currentPath) {
  ...
  return `
    ...
    if (link.children) {
      return `
                        <div class="relative group">
                            <button class="flex items-center gap-1 text-xs font-medium tracking-widest uppercase hover:text-white transition-colors ${isParentActive(link.children)}">
                                ${link.label}
                            ...
                            <div class="absolute left-0 top-full pt-4 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform group-hover:translate-y-0 translate-y-2">
                                <div class="bg-neutral-900 border border-white/10 rounded-lg p-2 shadow-xl">
                                    ${link.children.map(child => `
                                        <a href="${child.path}" class="block px-4 py-2 text-sm text-neutral-400 hover:text-white hover:bg-white/5 rounded-md transition-colors ${isActive(child.path)}">
                                            ${child.label}
                                        </a>
                                    `).join('')}
                                </div>
    ...
    return `
                    <a href="${link.path}" class="text-xs font-medium tracking-widest uppercase hover:text-white transition-colors ${isActive(link.path)}">
                        ${link.label}
                    </a>
                `
  }).join('')}
      ...
      return `
                    <div class="py-3 border-b border-white/5">
                        ...
                        <div class="pl-4 space-y-2">
                            ${link.children.map(child => `
                                <a href="${child.path}" class="block py-2 text-sm ${isActive(child.path)} hover:text-white transition-colors">
                                    ${child.label}
                                </a>
                            `).join('')}
                        </div>
    ...
    return `
                <a href="${link.path}" class="block py-3 text-sm font-medium ${isActive(link.path)} hover:text-white transition-colors border-b border-white/5 last:border-0">
                    ${link.label}
                </a>
              `
  }).join('')}
